...
async def login_with_google(token_data: AccessToken):
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail=f"Failed to validate access token: {str(e)}")

    if "error" in token_info:
        raise HTTPException(status_code=400, detail=f"Invalid access token: {token_info['error_description']}")
